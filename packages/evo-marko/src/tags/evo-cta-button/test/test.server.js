...
describe("cta-button", () => {
  it("renders basic cta button", async () => {
    ...
    const textEl = getByText(input.content.text);
  ...
  it("renders small cta button", async () => {
    ...
    const { getByText } = await render(template, input);
    expect(getByText(input.content.text).closest(".cta-btn")).toMatchSnapshot();
  });

  it("renders large cta button", async () => {
    const input = mock.Large;
    const { getByText } = await render(template, input);
    expect(getByText(input.content.text)).toMatchSnapshot();
  });
