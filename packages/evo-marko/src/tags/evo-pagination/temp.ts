...
interface PaginationInput extends Omit<Marko.HTML.Nav, `on${string}`> {
    ...
    a11yPreviousText?: Marko.HTMLAttributes["aria-label"];
    a11yNextText?: Marko.HTMLAttributes["aria-label"];
    onSelect?: (event:Event, target: HTMLElement, index:number) => void;
    onNext?: (event:Event, target: HTMLElement, index:number) => void;
    onPrevious?: (event:Event, target: HTMLElement, index:number) => void;
}
